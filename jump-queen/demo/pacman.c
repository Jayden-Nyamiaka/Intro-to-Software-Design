...
void check_offscreen(body_t *pacman) {
  ...
  if (center.x - PACMAN_RADIUS > WINDOW_WIDTH) {
    ...
    body_set_centroid(pacman,
                      (vector_t){WINDOW_WIDTH - PACMAN_RADIUS, center.y});
  ...
  if (center.y - PACMAN_RADIUS > WINDOW_HEIGHT) {
    ...
    body_set_centroid(pacman,
                      (vector_t){center.x, WINDOW_HEIGHT - PACMAN_RADIUS});
  }
}

void pacman_move(body_t *pacman, key_event_type_t type, double held_time,
                 double orientation, vector_t direction) {
  ...
  if (type == KEY_PRESSED) {
    ...
    velocity = vec_add(
        velocity, vec_multiply(PACMAN_ACCELERATION * held_time, direction));
...
state_t *emscripten_init() {
  ...
  state->time_since_pellet = 0;
  // adds background
  state->background = body_init(
      make_rectangle(VEC_ZERO, (vector_t){WINDOW_WIDTH, WINDOW_HEIGHT}), 0,
      BACKGROUND_COLOR);
  scene_add_body(state->scene, state->background);
  // adds pacman
  state->pacman = body_init(draw_pacman(PACMAN_RADIUS, WINDOW_CENTER), 0,
                            PACMAN_PELLET_COLOR);
