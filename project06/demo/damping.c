...
state_t *emscripten_init() {
  ...
  for (double x = 0; x < WINDOW_WIDTH; x += 2 * BALL_RADIUS) {
    ...
    double position_y =
        0.5 * WINDOW_HEIGHT * (1 + (x / WINDOW_WIDTH) * cos(angle * x));
    vector_t center = {position_x, position_y};
    body_t *real_ball = make_real_ball(center);
    body_t *invisiball =
        make_invisiball((vector_t){center.x, 0.5 * WINDOW_HEIGHT});
  ...
  for (size_t i = 1; i < scene_bodies(state->scene); i += 2) {
    ...
    list_add(bodies, scene_get_body(state->scene, i + 1));
    create_spring(state->scene, STIFFNESS_K_CONSTANT, bodies);
    bodies = list_init(1, NULL);
    list_add(bodies, scene_get_body(state->scene, i));
    create_drag(state->scene, DRAG_GAMMA_CONSTANT, bodies);
  }
  return state;
}
