...
const vector_t ROTATION_BOUNDS0 = {1, 10};
...
vec_list_t *make_star0(size_t sides, double inner_r, double outer_r,
...
state_t *emscripten_init() {
  ...
  for (size_t i = 0; i < STAR_NUM; i++) {
    ...
    cur_state->rotations[i] =
        r_sign() * r_int(ROTATION_BOUNDS0.x, ROTATION_BOUNDS0.y);
  }
  ...
  for (size_t i = 0; i < STAR_NUM; i++) {
    ...
    cur_state->stars[i] =
        make_star0(STAR_SIDES, STAR_SIZE.x, STAR_SIZE.y, 0, random_position);
  }
...
void new_rotation(state_t *state, size_t star_index) {
  state->rotations[star_index] = r_int(ROTATION_BOUNDS0.x, ROTATION_BOUNDS0.y) *
