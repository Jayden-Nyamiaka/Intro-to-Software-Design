...
void add_star(state_t *state, double dt) {
  ...
  if (state->time_since_star > STAR_TIME_INTERVAL) {
    ...
    body_t *star = body_init(
      make_star(state->sides, INNER_RADIUS, OUTER_RADIUS, start_pos, 0), MASS, r_color());
    body_set_velocity(star, (vector_t){VELOCITY_X, 0});
    body_set_angular_velocity(star, ROTATION_OF_STARS * r_double(ELASTICITY_BOUNDS.x, ELASTICITY_BOUNDS.y));
    scene_add_body(state->scene, star);
    // adds force (earth gravity) to new star
    create_earth_gravity(state->scene, LITTLE_G_CONSTANT, star);
    state->sides++;
...
void star_check_remove(scene_t *scene) {
  list_t *shape = body_get_shape(scene_get_body(scene, 0));
  int in_screen = 1;
